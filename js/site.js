(function () {
  ...
  function noteCardHtml(note) {
    const img = note.coverImage
      ...
      : '';
    return (
      '<a class="note-card reveal" href="nota.html?slug=' + encodeURIComponent(note.slug) + '">' +
        '<div class="note-card__media">' + img + '</div>' +
        '<div class="note-card__body">' +
          '<span class="badge">' + esc(note.category) + '</span>' +
  ...
  async function initArticle() {
    ...
    root.innerHTML =
      '<div class="article__eyebrow"><span class="badge">' + esc(note.category) + '</span></div>' +
  ...
  function observeReveal() {
    ...
    if (!('IntersectionObserver' in window) || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      els.forEach(function (elm) { elm.classList.add('is-visible'); });
      return;
    }
    ...
    els.forEach(function (elm) { io.observe(elm); });
  }
